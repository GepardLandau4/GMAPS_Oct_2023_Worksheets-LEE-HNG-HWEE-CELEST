...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        if (IsCaptain)
        {
            DebugExtension.DebugArrow(transform.position, Vector3.forward, Color.blue);
            DebugExtension.DebugArrow(transform.position, OtherPlayer.transform.position, Color.black);
            Debug.Log(Mathf.Acos((Vector3.Dot(OtherPlayer.transform.position, Vector3.forward)/OtherPlayer.transform.position.magnitude)
